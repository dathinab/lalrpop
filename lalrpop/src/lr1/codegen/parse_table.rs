...
impl<'ascent, 'grammar, W: Write> CodeGenerator<'ascent, 'grammar, W, TableDriven<'grammar>> {
    ...
    fn write_machine_definition(&mut self) -> io::Result<()> {
        ...
        rust!(self.out, "{p}expected_tokens(state as usize)", p = self.prefix);
        rust!(self.out, "}}");
        ...
        rust!(self.out, "states: &mut Vec<{state_type}>,", state_type = state_type);
        rust!(
            self.out,
    ...
    fn write_value_type_defn(&mut self) -> io::Result<()> {
        ...
        rust!(
            ...
            "pub enum {}Symbol<{}>",
            self.prefix,
            Sep(", ", &self.grammar.type_parameters)
        );

        if !self.grammar.where_clauses.is_empty() {
            rust!(
                self.out,
                " where {}",
                Sep(", ", &self.grammar.where_clauses)
            );
        }
        ...
        // make a dummy variant with phantomdata
        rust!(
            self.out,
            "VariantPhantom({phantom})",
            phantom = phantom_data_type,
        );
